...
Player::~Player() { delete playerSprite; }
...
CollisionInfo Player::collisionX(const Map& map) const {
    ...
    const sf::Vector2f topLeftCage = {rect.left / cageSize,
                                      rect.top / cageSize};
    const sf::Vector2f bottomRightCage = {(rect.left + rect.width) / cageSize,
                                          (rect.top + rect.height) / cageSize};

    CollisionInfo result = singleCollisionX(topLeftCage.y, bottomRightCage.y,
                                            map, topLeftCage.x, cageSize);

    if (result.collisionSide != 0) {
        return result;
    }

    result = singleCollisionX(topLeftCage.y, bottomRightCage.y, map,
                              bottomRightCage.x, cageSize);
    return result;
}

CollisionInfo Player::singleCollisionX(const float top, const float bottom,
                                       const Map& map, const int x,
                                       const int& cageSize) const {
...
CollisionInfo Player::collisionY(const Map& map) const {
    ...
    CollisionInfo result = singleCollisionY(topLeftCage.x, bottomRightCage.x,
                                            map, bottomRightCage.y, cageSize);

    if (result.collisionSide != 0) {
        return result;
    }

    result = singleCollisionY(topLeftCage.x, bottomRightCage.x, map,
                              topLeftCage.y, cageSize);

    return result;
}

CollisionInfo Player::singleCollisionY(const float left, const float right,
                                       const Map& map, const int y,
                                       const int& cageSize) const {
    CollisionInfo result;
...
bool Player::isTopOfStairs(const int curCage) { return (curCage == 4); }
...
void Player::setVel(const sf::Vector2f vel) { plVelocity = vel; }

sf::Vector2f Player::getVel() const { return plVelocity; }

bool Player::isOnGround() const { return onGround; }

bool Player::isStairsAvailable() const { return stairsAvailable; }

bool Player::isOnStairs() const { return onStairs; }

sf::Sprite* Player::getSprite(const sf::Vector2i offset) const {
    playerSprite->setPosition(
        sf::Vector2f(rect.left - offset.x, rect.top - offset.y));
    return playerSprite;
}
...
sf::RectangleShape Player::dbgSprite(const sf::Vector2i offset) const {
    ...
    dbg.setPosition(
        sf::Vector2f(cageSize * (rect.left / cageSize) - offset.x + thick,
                     cageSize * (rect.top / cageSize) - offset.y + thick));
