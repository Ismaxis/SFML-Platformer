...
std::pair<std::string, std::string> startButtonPaths{
    "Textures/startButton.png", "Textures/startButtonActive.png"};

std::pair<std::string, std::string> exitButtonPaths{
    "Textures/exitButton.png", "Textures/exitButtonActive.png"};
...
int main() {
    ...
    sf::RenderWindow window(
        sf::VideoMode(winPixelSize.x - 1u, winPixelSize.y - 1u), "Game",
        sf::Style::Default);
    ...
    while (window.isOpen()) {
        ...
        if (updateCode == START_LEVEL) {
            delete curLayout;
            curLayout = new Level(mapPath, mapSheetPath, playerTexturePath,
                                  winPixelSize);
            continue;
        }
