...
CollisionInfo Player::singleCollisionX(const float top, const float bottom, const Map& map, const int x,
...
CollisionInfo Player::singleCollisionY(const float left, const float right, const Map& map, const int y,
                                       const int& cageSize) const
{
    CollisionInfo result;
...
sf::RectangleShape Player::dbgSprite(const sf::Vector2i offset) const
{
    ...
    dbg.setPosition(sf::Vector2f(cageSize * (rect.left / cageSize) - offset.x + thick,
